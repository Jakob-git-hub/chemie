  ...
  return (
    <button
      ...
      className={cn(
        ...
        'hover:z-10 hover:scale-[1.12] hover:shadow-xl focus:z-10 focus:outline-none focus:ring-2 focus:ring-offset-1',
        'ring-offset-background',
        dimmed ? 'opacity-25 saturate-50' : 'opacity-100',
        highlighted && !dimmed && 'ring-2 ring-foreground ring-offset-2 shadow-lg'
      )}
      aria-label={`${element.name}, Ordnungszahl ${element.number}`}
    >
      <span className="text-[8px] font-semibold leading-none sm:text-[10px] md:text-xs">
        {element.number}
      </span>
      <span className="text-center text-sm font-bold leading-none sm:text-base lg:text-lg">
        {element.symbol}
      </span>
