// AI-first Chemie-Löser mit heuristischem Fallback.
// Die KI soll alle typischen Schulchemie-Aufgaben möglichst vollständig lösen.
...
// AI controls
const useAICheck = document.getElementById('use-ai');
const apiKeyInput = document.getElementById('api-key');
const modelSelect = document.getElementById('model');

async function callOpenAI(problemText, apiKey, model) {
  const systemPrompt = [
  ...
  const userPrompt = `Aufgabe:\n${problemText}\n\nGib die bestmögliche vollständige chemische Lösung. Wenn es mehrere mögliche Interpretationen gibt, wähle die wahrscheinlichste und erkläre deine Annahmen.`;
...
btn.addEventListener('click', async () => {
  ...
  if (!text) {
    stepsEl.textContent = 'Bitte eine Aufgabenstellung eingeben.';
    ansEl.textContent = '—';
    return;
  }

  stepsEl.textContent = 'Analysiere...';
  ansEl.textContent = '';
  ...
  if (useAICheck && useAICheck.checked) {
    ...
    if (!key) {
      stepsEl.textContent = 'AI-Modus aktiviert, aber kein API-Key angegeben. Bitte API-Key eintragen.';
      return;
    }
    try {
      stepsEl.textContent = 'Frage die KI an... (kann einige Sekunden dauern)';
      const aiResp = await callOpenAI(text, key, model);
      if (aiResp) {
        const rendered = renderAiResponse(aiResp, 'KI-Antwort konnte nicht interpretiert werden.');
        stepsEl.textContent = rendered;
        ansEl.textContent = typeof aiResp.final_answer === 'string' ? aiResp.final_answer : (typeof aiResp.answer === 'string' ? aiResp.answer : '');
      } else {
        stepsEl.textContent = 'Leere oder unlesbare Antwort von der KI. Verwende lokale Heuristik.';
        const {steps, answer} = generateSolution(text);
        stepsEl.textContent = steps;
        ansEl.textContent = answer;
      }
    } catch (err) {
      stepsEl.textContent = `Fehler bei KI-Anfrage: ${err.message}. Verwende lokale Heuristik.`;
      const {steps, answer} = generateSolution(text);
      stepsEl.textContent = steps;
      ansEl.textContent = answer;
    }
...
clearBtn.addEventListener('click', () => {
  ta.value = '';
  stepsEl.textContent = '— noch keine Aufgabe eingegeben —';
  ansEl.textContent = '— —';
  if (apiKeyInput) apiKeyInput.value = '';
  if (useAICheck) useAICheck.checked = true;
});

// Beispieltexte (in dev mode) – kann entfernt werden
// ta.value = 'Wie viele Mol sind in 18 g H2O?';